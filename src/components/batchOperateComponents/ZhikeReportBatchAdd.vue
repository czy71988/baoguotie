<template>
  <div class="inner-page">
    ...
    <el-table :data="tableList" border highlight fit style="width: 100%">
      ...
      <el-table-column label="合同影印件" width="120" align="center">
        <template slot-scope="scope">
          <el-image
            style="width: 120px; height: 63px"
            v-show="scope.row.contractimg"
    ...
    <zhike-report-batch-add-edit v-model="showEdit" :ruleForm="editingData.row" @submitOk="handleEditOkSumbit"></zhike-report-batch-add-edit>
  </div>
</template>
...
import ZhikeReportBatchAddEdit from './ZhikeReportBatchAddEdit.vue'
export default {
  components: {
    ZhikeReportBatchAddEdit
  },
  data () {
    return {
      tableList: [],
  ...
  methods: {
    ...
    hanndleNewRow (index, row) {
      this.createReport(index + 1, Object.assign({}, row, {
        contractimg: ''
      }), (row) => {

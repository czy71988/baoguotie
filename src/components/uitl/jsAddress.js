const addressInit = function (_carea, _cmbProvince, _cmbCity, _cmbArea, defaultarea1, defaultProvince, defaultCity, defaultArea) {
...
var provinceList = [
  ...
  { _area: '西南地区',
    Allcity: [
      ...
      { name: '重庆',
        cityList: [
          { name: '江津市', areaList: ['万州区', '涪陵区', '渝中区', '大渡口区', '江北区', '沙坪坝区', '九龙坡区', '南岸区', '北碚区', '万盛区', '双桥区', '渝北区', '巴南区', '黔江区', '长寿区'] },
          { name: '合川市', areaList: ['綦江县', '潼南县', '铜梁县', '大足县', '荣昌县', '璧山县', '梁平县', '城口县', '丰都县', '垫江县', '武隆县', '忠　县', '开　县', '云阳县', '奉节县', '巫山县', '巫溪县', '石柱土家族自治县', '秀山土家族苗族自治县', '酉阳土家族苗族自治县', '彭水苗族土家族自治县'] },
...
let regionList = []
let regionMapProvince = null
let provinceMapCity = null
let cityMapArea = null
// 获取地区
function getRegionList () {
  if (regionList && regionList.length > 0) {
    return regionList
  }
  regionMapProvince = provinceList.reduce((result, item) => {
    regionList.push(item._area)
...
// 获取城市列表根据省份
function getCityListFromProvinceName (provinceName) {
  if (!provinceMapCity || !provinceMapCity[provinceName]) {
    return null
  }
  if (!cityMapArea) cityMapArea = {}
  return provinceMapCity[provinceName].map(item => {
    cityMapArea[item.name] = item.areaList
    return item.name
  })
}
// 获取地区根据城市
function getAreaListFromCityName (cityName) {
  if (!cityMapArea || !cityMapArea[cityName]) {
    return null
  }
  return cityMapArea[cityName]
}
